...
enum ProcessorImpl<'h, W: io::Write> {
    LazyLoading(HtmlRewriter<'static, WriterOutputSink<&'h mut W>>),
    HtmlEscape(Escaper<&'h mut W>),
}

impl ProcessorType {
    fn build<'w, W: io::Write>(&self, output: &'w mut W) -> ProcessorImpl<'w, W> {
        match self {
